  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-[#2a2a3a] bg-[#0f0f13]/90 backdrop-blur-md">
        {/* 3-column grid: left | center | right — guarantees logo stays centered */}
        <div className="mx-auto grid h-16 max-w-7xl grid-cols-3 items-center px-4">
          ...
          <div className="flex items-center justify-end gap-2">
            {isSignedIn ? (
              ...
              <>
                {/* Log in: hidden on mobile, visible on sm+ */}
                <Link
                  href="/sign-in"
                  className="hidden sm:block rounded-full border border-[#2a2a3a] px-4 py-1.5 text-sm font-medium text-[#f0f0f5] hover:border-[#ff3b5c] transition-colors"
                >
                  Log in
                </Link>
                <Link
                  href="/sign-up"
                  className="rounded-full bg-[#ff3b5c] px-4 py-1.5 text-sm font-semibold text-white hover:bg-[#e0304f] transition-colors whitespace-nowrap"
                >
                  Sign up
                </Link>
              </>
